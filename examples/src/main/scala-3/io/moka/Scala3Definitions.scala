...
/** Scala 3-only style: `Moka.generateFields[T]` in the companion, no @moka
  * annotation. This is the cleanest way to use moka on Scala 3, but it does
  * not cross-compile: the Scala 2 macro only rewrites the placeholder val
  * when the case class is annotated with @moka.
  */
object Scala3Definitions {
  ...
  final case class BsonFields(@BsonProperty("renamed") a: Int, @bsonField("znamed") b: Int)
  object BsonFields {
